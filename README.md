...
Optional JSON under `data/` (SKU costs, exclusions, thresholds) is **written with the filesystem**. **Vercel serverless does not offer a durable writable project disk** like your laptop—edits from the Costs UI **will not persist** reliably in production. For hosted deploys, use **Google Sheet CSV sync** (`SP_SKU_COSTS_SHEET_CSV_URL`) or change storage to a database / blob store later.
